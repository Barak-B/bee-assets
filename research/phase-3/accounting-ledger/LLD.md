...
- **Inbound:**
  ...
  - `[[knowledge-base/il-self-employed-tax]]` `[PLANNED]` — feeds ביטוח לאומי forecasting (מקדמות = 0% per LD-2, not forecast)
- **Outbound:**
  - `[[Invoice_Maven]]` — export pack targets IM's CSV import format (per LD-1; serves invoicing AND accounting). NO third-party software bridge.
...
model CustomerInvoice {
  ...
  invoiceNumber   String   @unique                             // LD-4: CONTINUOUS monotonic, NO year reset.
                                                               //   Generator = MAX(invoiceNumber::int)+1; seed = highest existing in Invoice Maven (Phase A0).
...
model TaxFiling {
  id              String   @id @default(cuid())
  kind            String                                       // 'vat' | 'bituach-leumi'  (income-advance dropped per LD-2: 0% מקדמות)
...
flowchart TD
  ...
  post --> match{"Counter-entry exists?<br/>(e.g. CustomerInvoice ↔ BankTransaction by customer + amount ±5% + date postedAt→+90d)"}
  ...
  subgraph REPORTS["Step 7, 9-11 — periodic + outbound"]
    vat["Monthly cron 1st of month (LD-3):<br/>generate VAT report (דו""ח מע""מ)<br/>aggregate by monthly period boundaries"]
    ...
    imexport["Tier 0 — monthly export pack:<br/>customers/suppliers/transactions CSV → INVOICE_MAVEN_EXPORT_DIR"]
  end
...
2. Match criteria (canonical window, same as §4.2 code): same entityId · amount ±5% · date window `postedAt → postedAt+90d` · currency. This single tolerance is also what 53/B §6 and the spine §3.1 reference — do not diverge.
...
### 3.6 The Invoice Maven export pack (monthly cron) — per LD-1

LD-1: Invoice Maven serves BOTH invoicing AND accounting. There is **no third-party
software bridge** (no Hashavshevet/Rivhit/Priority). The export pack targets Invoice
Maven's CSV **import** format so balances/transactions round-trip into IM, where the
רו"ח already works.

Lands in `INVOICE_MAVEN_EXPORT_DIR`:

```
2026-06/
  README.md                       — period summary, totals, anomalies flagged
  customers.csv                   — IM customer import schema (entity + opening/closing balance)
  suppliers.csv                   — IM supplier import schema
  transactions.csv                — IM transaction import schema (date, entity, side, amount, ref, description)
  vat-report.pdf                  — pre-filled monthly דו""ח מע""מ (read-only; רו"ח files)
  ar-aging.xlsx                   — AR aging snapshot at period end (operator visibility)
...
The three IM CSVs (`customers.csv`/`suppliers.csv`/`transactions.csv`) are the canonical
hand-off (decisions §C4). The xlsx/pdf files are operator-facing extras. Exact IM column
layout is locked once Barak provides a real IM export sample (artifact OB-2). Sent to
`ACCOUNTANT_EMAIL` once finalized (Barak presses approve, same shape as `sendProposal`).
...
🧾 *מסים*
  דו""ח מע""מ הבא (חודשי): 15/7 (תקופה 6) · ייצור: ~₪48,000, ניכוי: ~₪33,500 → לתשלום: ~₪14,500
  (אין מקדמות מס הכנסה — 0% לפי LD-2)
...
// --- Source adapter: signed BankTransaction (53/A) → positive amount + side ---
// BankTransaction.amountCents is SIGNED (+ = inflow). postLedgerEntry requires a
// POSITIVE amount + explicit side, so the hook MUST strip the sign and map it.
// Without this adapter a naive hook passing the signed value throws on every outflow.
export function bankTxToLedgerArgs(tx: { id: string; amountCents: bigint; counterpartyNorm: string; valueDate: Date; customerId?: string | null }): PostLedgerArgs {
  const inflow = tx.amountCents >= 0n;
  return {
    postedAt: tx.valueDate,
    // inflow from a customer = they paid us (CREDIT customer); outflow = we paid (DEBIT)
    entityKind: tx.customerId ? "CUSTOMER" : "BANK_INTERNAL",
    entityId: tx.customerId ?? "BANK_INTERNAL",
    side: inflow ? "CREDIT" : "DEBIT",
    amountCents: tx.amountCents < 0n ? -tx.amountCents : tx.amountCents,   // abs(), BigInt-safe
    description: tx.counterpartyNorm,
    source: { bankTxId: tx.id },
  };
}
```
...
0 9 1 * *              — monthly: generate "Barak's books" ⚡ + assemble Invoice Maven export pack draft
0 9 1 * *              — monthly (LD-3): VAT report draft + alert Barak ahead of the 15th due date
0 10 * * 0             — weekly: AR collection nudge drafts for >30d items (drafts to drafts-group, Barak picks)
```
> מקדמות forecast cron REMOVED per LD-2 (0% obligation — nothing to forecast).
> The two `0 9 1 * *` jobs run in one monthly handler (books ⚡ + VAT draft) — listed
> separately for clarity; merge in implementation.
...
| **מע"מ חודשי (monthly VAT)** | `TaxFiling` model + monthly cron; period boundaries enforced by config `VAT_PERIOD_MONTHS=1` (LD-3). |
| **מקדמות מס הכנסה** | **N/A — 0% per LD-2.** No מקדמות cron, no `TaxFiling.kind='income-advance'`. |
| **ביטוח לאומי לעצמאי** | Quarterly TaxFiling rows pre-populated (`kind='bituach-leumi'`). |
...
- **Double-entry full bookkeeping** (חשבונאות כפולה מלאה with general-ledger accounts). What we have is *subsidiary-ledger* style (per-entity), which is what BEE and רו"ח actually use day-to-day. The general ledger lives in **Invoice Maven** (LD-1), which the רו"ח works in.
- **A third-party accounting-software bridge** (Hashavshevet / Rivhit / Priority). **Resolved per LD-1: NOT built.** Invoice Maven serves both invoicing and accounting; §3.6 exports to IM's CSV import format, not to any of those.
